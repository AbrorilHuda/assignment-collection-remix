...
export default function UploadForm() {
  ...
  return (
    ...
    <h1 className="text-2xl font-bold mb-10 text-center">Form Pengumpulan Tugas</h1>
    <MyAlert color="warning" hide={hiden.status} message={hiden.message}/>
    <Button onPress={() => setHiden({status: false})} variant="ghost" size="sm" className={`bg-orange-500 ${!hiden.status ? "hidden": ""}`} >close message</Button>
    <Form onSubmit={handleSubmit} className="space-y-4 p-5 items-center">
      <Input
      required
      ...
      <Input
        className="max-w-xs bg-gray-800 border rounded-xl"
        value={nim}
        onChange={(e: ChangeEvent<HTMLInputElement>) => setNim(e.target.value)}
        fullWidth
      ...
      <Button type="submit" fullWidth className="bg-blue-800 rounded-md hover:bg-blue-950" disabled={isLoading}>
